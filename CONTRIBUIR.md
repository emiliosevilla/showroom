...
1. **Fork del repo**: entra a este repositorio en GitHub y pulsa "Fork" (arriba a la
   derecha). Se crea una copia bajo tu cuenta: `tuusuario/showroom`.

2. **Clona tu fork** (no el original):
   ```bash
   git clone https://github.com/tuusuario/showroom.git
   cd showroom
   ```
...
histórico `.git`:
```bash
git worktree add ../showroom-feature-x -b feature/nombre-descriptivo upstream/dev
```
Así `showroom/` se queda en `dev` y `showroom-feature-x/` tiene la rama de trabajo,
...
- **head repository**: `tuusuario/showroom`, **compare**: `feature/nombre-descriptivo`
...
| Quiero... | Comando |
|---|---|
| Configurar el remoto original (una vez) | `git remote add upstream https://github.com/emiliosevilla/showroom.git` |
...
| Empezar una tarea nueva en carpeta aparte (worktree) | `git worktree add ../showroom-feature-x -b feature/x upstream/dev` |
| Subir mis cambios a mi fork | `git push origin feature/x` |
| Abrir el PR | En GitHub: fork → "Compare & pull request" (base: `emiliosevilla/showroom`/`dev`) |
| Actualizar un PR ya abierto | Nuevos commits en la misma rama + `git push origin feature/x` |
| Quitar un worktree ya usado | `git worktree remove ../showroom-feature-x` |
